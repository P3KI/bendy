use std::{fs::read, io::Read as _, path::PathBuf};
...
fn main() -> Result<()> {
    ...
    if args.file_paths.is_empty() {
        ...
        pretty_print(input.as_slice(), as_string_literal, "stdin")?;
    } else {
        for p in args.file_paths {
            let contents = read(&p).context(format!("Could not read file from path: {:?}", &p))?;
            pretty_print(
                contents.as_slice(),
                as_string_literal,
...
fn pretty_print(i: &[u8], as_string_literal: bool, source: &str) -> Result<()> {
    let i = Inspectable::try_from(i).context(format!("Could not parse {:?} as bencode", source))?;
    println!(
        "{}",
